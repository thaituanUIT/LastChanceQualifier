...
let tetrominoColors = ['purple','cyan','blue','yellow','orange','green','red', 'pink', 'lime', 'brown'];
...
function updateScore(points) {
    ...
    if (score % 100 === 0) {
        ...
        // Clear the old interval and set a new one with a shorter delay
        clearInterval(gameInterval);
        gameInterval = setInterval(function(){
            if(winOrLose != "Game Over"){
                MoveTetrominoDown();
            }
        }, 1000 / level); // The delay is 1000ms divided by the level, so it decreases as the level increases
    }
}
